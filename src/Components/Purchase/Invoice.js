...
const Invoice = () => {
  ...
  const fetchInvoices = async () => {
    try {
      const response = await axios.get('http://localhost:8080/api/purchase/invoices');
  ...
  const handleDelete = async (record) => {
    try {
      await axios.delete(`http://localhost:8080/api/purchase/invoices/${record.id}`);
      fetchInvoices();
    } catch (error) {
      console.log(error);
    }
  ...
  const handleModalSave = async () => {
    try {
      const response = await axios.put(`http://localhost:8080/api/purchase/invoices/${currentInvoice.id}`, currentInvoice);
  ...
  return (
    <div>
      <div style={{ marginBottom: 16 }}>
        <Button type="primary" onClick={() => history.push('./add-invoice')}>
          Add Invoice
        </Button>
      </div>
          ...
          <Form layout="vertical">
            ...
            <Form.Item label="Shipping Charges">
              <Input
                ...
                onChange={(e) => handleFormChange('shippingCharges', e.target.value)}
              />
            </Form.Item>
